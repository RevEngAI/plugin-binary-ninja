...
class MatchFunctions(MatchFeature):
    ...
    def match_functions(self, bv: BinaryView, options: Dict[str, Any]) -> List[Dict]:
        ...
        try:
            ...
            log_info(f"RevEng.AI | Matching {len(function_ids)} functions in analysis {analysis_id}")
            ...
            start_body = revengai.StartMatchingForAnalysisInputBody.from_dict({
                "min_similarity": 0,
                "filters": filters,
            ...
            with self.config.create_api_client() as api_client:
                analyses_core_instance = revengai.AnalysesCoreApi(api_client)
                analyses_core_instance.start_analysis_function_matching(analysis_id, start_body)
            ...
            while True:
                ...
                with self.config.create_api_client() as api_client:
                    analyses_core_instance = revengai.AnalysesCoreApi(api_client)
                    status = analyses_core_instance.get_analysis_function_matching_status(analysis_id)
            ...
            with self.config.create_api_client() as api_client:
                analyses_core_instance = revengai.AnalysesCoreApi(api_client)
                matches_result = analyses_core_instance.get_analysis_function_matches(analysis_id)
            ...
            for function_by_distance in (matches_result.matches or []):
                try:
                    ...
                    if function:
                        line["function_name"] = function.name
                        line["function_address"] = function.start
                    if not function:
                        function = bv.get_function_at(func_addr + bv.image_base)
                        log_info(f"RevEng.AI | Function: {function} at {func_addr + bv.image_base:x}")
                        if function:
                            line["function_name"] = function.name
                            line["function_address"] = function.start
                        else:
                    ...
                    if function_by_distance.matched_functions[0].confidence < confidence_threshold:
